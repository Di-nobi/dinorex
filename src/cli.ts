...
function checkApiKey(options: ScanOptions | GenerateOptions): void {
  const isAnthropic =
    (options as ScanOptions).provider === "anthropic" ||
...
program
  ...
  .action(async (directory = ".", options: ScanOptions) => {
    ...
    const isAnthropic =
      options.provider === "anthropic" || process.env.DINOREX_PROVIDER === "anthropic";
    ...
    if (!total) {
      s1.fail(
        chalk.red("No API files found. Make sure you're in an Express/Nest/Fastify project.")
      );
      process.exit(1);
    }
    s1.succeed(
    ...
    if (stored?.spec && stored?.hashes) {
      ...
      const hasChanges =
        diff.newFiles.length || diff.changedFiles.length || diff.removedFiles.length;
      ...
      if (hasChanges) {
        ...
        try {
          ...
          s2.fail(chalk.red((e as Error).message));
          process.exit(1);
        }
      } else {
        spec = stored.spec;
      ...
      try {
        ...
        saveStore(targetDir, { spec, hashes, lastScan: new Date().toISOString() });
        s2.succeed(
          chalk.cyan("Analysis complete — ") +
            chalk.white(
              `${spec.collections.reduce((a: number, c: { endpoints: unknown[] }) => a + c.endpoints.length, 0)} endpoints across ${spec.collections.length} collections`
            )
        );
      } catch (e) {
        s2.fail(chalk.red((e as Error).message));
