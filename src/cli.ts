...
function printUser(auth: AuthData): void {
...
async function runSignup(): Promise<AuthData> {
  ...
  try {
    const res = await apiRequest<{ token: string; user: AuthData["user"]; message: string }>(
      "/auth/signup",
      { method: "POST", body: { name, email, password } }
    );

    const auth: AuthData = { token: res.token, user: res.user };
    saveAuth(auth);
...
async function runLogin(): Promise<AuthData> {
...
program
  ...
  .action(async (directory = ".", options: { port: string; open: boolean }) => {
    banner();
    ...
    try {
      ...
      s1.fail(chalk.red((e as Error).message));
      process.exit(1);
    }
    ...
    if (stored?.spec && stored?.hashes) {
      ...
      if (!hasChanges) {
        ...
        try {
          const res = await apiRequest<{
            success: boolean;
            spec: typeof spec;
            usage: { scansThisMonth: number; scanLimit: number | null; scansRemaining: number | null; plan: string };
          }>("/scan", {
            method: "POST",
            token: auth.token,
            body: {
          ...
          s2.fail(chalk.red((err as Error).message));
          process.exit(1);
        }
      }
    } else {
      ...
      try {
        ...
        const totalEndpoints = spec.collections.reduce(
          (a: number, c: { endpoints: unknown[] }) => a + c.endpoints.length, 0
        );
        s2.succeed(
          chalk.cyan(`${totalEndpoints} endpoints across ${spec.collections.length} collections — `) + usageStr
        );
      } catch (err) {
        s2.fail(chalk.red((err as Error).message));
        process.exit(1);
    ...
    try {
      ...
      s3.fail(chalk.red((e as Error).message));
      process.exit(1);
    }
